...
Go through the pull requests, search for a few matching terms of your idea and ensure it's not been done before. This saves you time trying an idea that may have been tried before, or it may inspire you to take an alternative approach.
...
The easiest part! Create the PR with an easy-to-follow description and ensure that CI is passing (ie the tests), if the StyleCI CI item is failing, dont panic.. that's just styling and it will automatically be fixed if Taylor merges your PR. (Or, take its fixes and apply then before hand) 
