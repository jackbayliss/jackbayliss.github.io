---
title: "Laracon USA 2026"
showToc: false
date: "2026-08-02"
tags: ['PHP', 'Laravel', 'Laracon 2026', 'Laracon USA 2026']
description: "Laracon USA 2026."
---

So, Laracon USA was only a few days ago, if you haven't had chance to watch it check out [Day 1](https://www.youtube.com/watch?v=0P-VMS7JMQY) and [Day 2](https://www.youtube.com/watch?v=vii6P0vJhTw)

## Talks 

There were many epic talks, and to be honest, I won't go over each one. But, there was a lot learned, such as JJ, how to prompt AI better, and take note of when you feel the code pushing back and of course AI! But, let us be wary that using AI doesn't NECESSARILY mean we learn faster. Also, lets not forget "Cleverness Is A Loan" every abstraction you do, is making it more difficult for the next dev that has to go through the code..

## Meeting people

I saw sooooo many people whos face I know from the internet...
Big man Taylor Otwell, Mathias Hansen of Geocodio, "mortal enemy" Luke Kuzmish, Andy Hinkle, Caleb White and Andrew Mast. I also got to meet many of the Laravel staff team, which is always cool. I realised I'm kind of an introvert, I guess mainly cause I work remotely, but come and speak to me and I open up! I also got to speak to a lot of US companies and other developers I don't know, whether it companies introducing themselves, discussions around Planetscale, how to do a PR, or generally what people are using Laravel for it was all awesome conversations.

## USA, USA!

I used Laracon as an excuse to take some holiday, and also took the Mrs, It was my first time in the states, and one of the longest flights of my life. (I'm a pretty bad flyer) But, Boston was awesome. I think I'd definitely go back, I still can't quite get my head around that fact a Big Gulp is $1.75 for like a litre. I got to eat all the food we don't get over here, Cheesecake Factory, Raising Canes... And Jersey Mikes. I must say the food was pretty good, though everything feels way expensive compared to here in the UK.  

## Obsession

I find when I go on Holiday I need to see what the team back in England are doing, and find myself dipping in and leaving a few comments here and there. Its the same for the Laravel Framework, one of the nights I found myself sorting a Monolog issue, which I probably could of left for someone else to pickup. I guess my thinking is, if not me, then who else? Though, I tried to refrain!

## Takeaways

- AI is a big big thing, and TBH it seems 90% of people use it
- Although, we are in the age of AI, we should really spend time considering an approach
- Probably go to the US again at some point? (Surprisingly it was fun!)
- Spend more time at booths speaking to people rather than sitting waiting for the next talk for an hour
